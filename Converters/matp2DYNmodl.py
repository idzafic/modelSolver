...
def writeDiagPowerTermInPolar(realTerms, busID : int, lblV, lblYMag, lblYAngle, isSlackTo, bZeroInjection : bool) -> None:
    if isSlackTo:
        if bZeroInjection:
            realTerms.append(f"\t{lblYMag}_{busID}_{busID}*{lblV}_{busID}_init*cos({lblYAngle}_{busID}_{busID})")
        else:
            realTerms.append(f"\t{lblYMag}_{busID}_{busID}*{lblV}_{busID}_init^2*cos({lblYAngle}_{busID}_{busID})")
    else:
        if bZeroInjection:
...
def writeDiagReactivePowerTermInPolar(imagTerms, busID : int, lblV, lblYMag, lblYAngle, isSlack, bZeroInjection : bool) -> None:
    if isSlack:
        if bZeroInjection:
            imagTerms.append(f"\t-{lblYMag}_{busID}_{busID}*{lblV}_{busID}_init*sin({lblYAngle}_{busID}_{busID})")
        else:
            imagTerms.append(f"\t-{lblYMag}_{busID}_{busID}*{lblV}_{busID}_init^2*sin({lblYAngle}_{busID}_{busID})")
    else:
        if bZeroInjection:
...
def writeOffDiagPowerTermInPolar(realTerms, busI, busJ, lblV, lblVangle, lblYMag, lblYAngle, isSlackTo, isSlackFrom, bFirst : bool):
    if isSlackTo:
        if not bFirst:
            realTerms.append(f" + {lblYMag}_{busI}_{busJ}*{lblV}_{busJ}_init*cos({lblVangle}_{busI}-{lblYAngle}_{busI}_{busJ}-{lblVangle}_{busJ}_init)")
        else:
            realTerms.append(f"{lblYMag}_{busI}_{busJ}*{lblV}_{busJ}_init*cos({lblVangle}_{busI}-{lblYAngle}_{busI}_{busJ}-{lblVangle}_{busJ}_init)")
    elif isSlackFrom:
        if not bFirst:
            realTerms.append(f" + {lblYMag}_{busI}_{busJ}*{lblV}_{busJ}*cos({lblVangle}_{busI}_init-{lblYAngle}_{busI}_{busJ}-{lblVangle}_{busJ})")
        else:
            realTerms.append(f"{lblYMag}_{busI}_{busJ}*{lblV}_{busJ}*cos({lblVangle}_{busI}_init-{lblYAngle}_{busI}_{busJ}-{lblVangle}_{busJ})")
    else:
        if not bFirst:
...
def writeOffDiagReactivePowerTermInPolar(imagTerms, busI, busJ, lblV, lblVangle, lblYMag, lblYAngle, isSlackTo, isSlackFrom, bFirst : bool):
    if isSlackTo:
        if not bFirst:
            imagTerms.append(f" + {lblYMag}_{busI}_{busJ}*{lblV}_{busJ}_init*sin({lblVangle}_{busI}-{lblYAngle}_{busI}_{busJ}-{lblVangle}_{busJ}_init)")
        else:
            imagTerms.append(f"{lblYMag}_{busI}_{busJ}*{lblV}_{busJ}_init*sin({lblVangle}_{busI}-{lblYAngle}_{busI}_{busJ}-{lblVangle}_{busJ}_init)")
    elif isSlackFrom:
        if not bFirst:
            imagTerms.append(f" + {lblYMag}_{busI}_{busJ}*{lblV}_{busJ}*sin({lblVangle}_{busI}_init-{lblYAngle}_{busI}_{busJ}-{lblVangle}_{busJ})")
        else:
            imagTerms.append(f"{lblYMag}_{busI}_{busJ}*{lblV}_{busJ}*sin({lblVangle}_{busI}_init-{lblYAngle}_{busI}_{busJ}-{lblVangle}_{busJ})")
    else:
        if not bFirst:
...
def writeDiagPowerTermInCurrentPolar(realTerms, busID : int, lblV, lblVangle, lblYMag, lblYAngle, isSlackTo, bZeroInjection : bool) -> None:
    if isSlackTo:
        if bZeroInjection:
            realTerms.append(f"\t{lblYMag}_{busID}_{busID}*{lblV}_{busID}_init*cos({lblYAngle}_{busID}_{busID}+{lblV}_{busID}_init)")
        else:
            realTerms.append(f"\t{lblYMag}_{busID}_{busID}*{lblV}_{busID}_init^2*cos({lblYAngle}_{busID}_{busID}+{lblV}_{busID}_init)")
    else:
        if bZeroInjection:
            realTerms.append(f"\t{lblYMag}_{busID}_{busID}*{lblV}_{busID}*cos({lblYAngle}_{busID}_{busID}+{lblVangle}_{busID})")
        else:
            realTerms.append(f"\t{lblYMag}_{busID}_{busID}*{lblV}_{busID}^2*cos({lblYAngle}_{busID}_{busID}+{lblVangle}_{busID})")


def writeOffDiagPowerTermInCurrentPolar(realTerms, busI, busJ, lblV, lblVangle, lblYMag, lblYAngle, isSlackTo, isSlackFrom, bFirst : bool):
    if isSlackTo:
        if not bFirst:
            realTerms.append(f" + {lblYMag}_{busI}_{busJ}*{lblV}_{busJ}_init*cos({lblYAngle}_{busI}_{busJ}+{lblVangle}_{busJ}_init)")
        else:
            realTerms.append(f"{lblYMag}_{busI}_{busJ}*{lblV}_{busJ}_init*cos({lblYAngle}_{busI}_{busJ}+{lblVangle}_{busJ}_init)")
    elif isSlackFrom:
        if not bFirst:
            realTerms.append(f" + {lblYMag}_{busI}_{busJ}*{lblV}_{busJ}*cos({lblYAngle}_{busI}_{busJ}+{lblVangle}_{busJ})")
        else:
            realTerms.append(f"{lblYMag}_{busI}_{busJ}*{lblV}_{busJ}*cos({lblYAngle}_{busI}_{busJ}+{lblVangle}_{busJ})")
    else:
        if not bFirst:
...
def writeDiagReactivePowerTermInCurrentPolar(imagTerms, busID : int, lblV, lblVangle, lblYMag, lblYAngle, isSlack, bZeroInjection : bool) -> None:
    if isSlack:
        if bZeroInjection:
            imagTerms.append(f"\t{lblYMag}_{busID}_{busID}*{lblV}_{busID}_init*sin({lblYAngle}_{busID}_{busID}+{lblV}_{busID}_init)")
        else:
            imagTerms.append(f"\t{lblYMag}_{busID}_{busID}*{lblV}_{busID}_init^2*sin({lblYAngle}_{busID}_{busID}+{lblV}_{busID}_init)")
    else:
        if bZeroInjection:
            imagTerms.append(f"\t{lblYMag}_{busID}_{busID}*{lblV}_{busID}*sin({lblYAngle}_{busID}_{busID}+{lblVangle}_{busID})")
        else:
            imagTerms.append(f"\t{lblYMag}_{busID}_{busID}*{lblV}_{busID}^2*sin({lblYAngle}_{busID}_{busID}+{lblVangle}_{busID})")


def writeOffDiagReactivePowerTermInCurrentPolar(imagTerms, busI, busJ, lblV, lblVangle, lblYMag, lblYAngle, isSlackTo, isSlackFrom, bFirst : bool):
    if isSlackTo:
        if not bFirst:
            imagTerms.append(f" + {lblYMag}_{busI}_{busJ}*{lblV}_{busJ}_init*sin({lblYAngle}_{busI}_{busJ}+{lblVangle}_{busJ}_init)")
        else:
            imagTerms.append(f"{lblYMag}_{busI}_{busJ}*{lblV}_{busJ}_init*sin({lblYAngle}_{busI}_{busJ}+{lblVangle}_{busJ}_init)")
    elif isSlackFrom:
        if not bFirst:
            imagTerms.append(f" + {lblYMag}_{busI}_{busJ}*{lblV}_{busJ}*sin({lblYAngle}_{busI}_{busJ}+{lblVangle}_{busJ})")
        else:
            imagTerms.append(f"{lblYMag}_{busI}_{busJ}*{lblV}_{busJ}*sin({lblYAngle}_{busI}_{busJ}+{lblVangle}_{busJ})")
    else:
        if not bFirst:
...
def writePostProcBlock(file, bus_id : int, P_inj : float, Q_inj : float, onOffPQSimulation : str):
    file.write(f"\n\tPostProc: \t//Simulate on/off or {onOffPQSimulation} PQ load\n")
    file.write("\t\tif t >= 0.5:\n")
    file.write("\t\t\tif t < 6:\n")
    file.write(f"\t\t\t\tP_{bus_id} = 0 \t//off between 0.5 and 6 sec\n")
    file.write(f"\t\t\t\tQ_{bus_id} = 0\n")
    file.write("\t\t\telse:\n")
    file.write(f"\t\t\t\tP_{bus_id} = {P_inj} \t//on again after 6 sec\n")
    file.write(f"\t\t\t\tQ_{bus_id} = {Q_inj}\n")
    file.write("\t\t\tend\n")
    file.write("\t\tend\n")

def writePostProcCurrentBlock(file, bus_id : int, P_inj : float, Q_inj : float, onOffPQSimulation : str, lblYAngle : str, lblYMag : str):
    file.write(f"\n\tPostProc: \t//Simulate on/off or {onOffPQSimulation} PQ load\n")
    file.write("\t\tif t >= 0.5:\n")
    file.write("\t\t\tif t < 6:\n")
    file.write(f"\t\t\t\t{lblYMag}_{bus_id}_{bus_id} = {lblYMag}_{bus_id}_{bus_id}_fault \t//off between 0.5 and 6 sec\n")
    file.write(f"\t\t\t\t{lblYAngle}_{bus_id}_{bus_id} = {lblYAngle}_{bus_id}_{bus_id}_fault\n")
    file.write("\t\t\telse:\n")
    file.write(f"\t\t\t\t{lblYMag}_{bus_id}_{bus_id} = {lblYMag}_{bus_id}_{bus_id}_reg \t//on again after 6 sec\n")
    file.write(f"\t\t\t\t{lblYAngle}_{bus_id}_{bus_id} = {lblYAngle}_{bus_id}_{bus_id}_reg\n")
    file.write("\t\t\tend\n")
    file.write("\t\tend\n")

def main():
    #  Determine if we're running in debug mode
    debug_mode = True #is_debugging()   # Reliable way to detect debugger
    ...
    config_file_path = "configDyn.xml"
    ...
    if debug_mode:
        ...
        matpower_file = "case30.m" # "case_ACTIVSg25k.m" "case118.m" #"case3375wp.m" #"case_SyntheticUSA.m" #"case6495rte.m" #"case6468rte.m" #"case300.m"
    ...
    try:
        ...
        rootDynamics = rootNode.find('dynamics')
    ...
    includeFrequencyRegulation = rootDynamics.find('includeFrequencyRegulation').text.strip().lower() == 'true'
    onOffPQSimulation = rootDynamics.find('onOffPQSimulation').text.strip().lower()
    limitGenPowers = rootDynamics.find('limitGenPowers').text.strip().lower() == 'true'
    plotMechPowerOnly = rootDynamics.find('plotMechPowerOnly').text.strip().lower() == 'true'
    usePIforFreq = rootDynamics.find('usePIforFreq').text.strip().lower() == 'true'
    useCurrentBalance = rootDynamics.find('useCurrentBalance').text.strip().lower() == 'true'
    kMechMult = float(rootDynamics.find('kMechMult').text)
    ...
    def parse_matrix_block(lines, start_index, elementType):
        nonlocal nBuses, nGens, nBranches, nGenPLimits
        LARGE = 1e40
        SMALL = -1e40
                ...
                if elementType == 0: #bus
                    ...
                    if row_data[7] < 0.1:  
                        if not detectedOffStatus: 
                            ...
                            print(f"WARNING! Detected off-service generator! First time at bus {int(row_data[0])}")
                        continue    #ignore gens with status == 0
                    if row_data[8] != 0 or row_data[9] != 0:    #check max and min active power
                        if row_data[8] <= row_data[1]:
                            print(f"WARNING! Detected generator on or over the P limit at bus {int(row_data[0])}")
                            row_data[8] = row_data[1]
                        row_data[8] /= baseMVA      #convert Pmax to p.u.
                        row_data[9] /= baseMVA      #convert Pmin to p.u.
                        row_data[8] *= kMechMult      #increase limit (to compensate losses)
     
                        nGenPLimits += 1
                        
                    else:
                        print(f"WARNING! Detected generator without P limits! Generator connected at bus {int(row_data[0])}")
    ...
    print(f"Detected {nBuses} buses, {nGens} generators, {nGenPLimits} generator limits, and {nBranches} branches.")
    ...
    if loadTransferSelect:
        resFileName = dmodl_output_path + "_lt_dyn.dmodl"
        ...
        resFileName = dmodl_output_path + "_dyn.dmodl"

    
    if genLimitsSelect:
        resFileName = dmodl_output_path + "_pv_dyn.dmodl"
    ...
    with open(resFileName, "w", encoding='utf-8') as file:
        ...
        for iBus in range(n):
            # IDz need fix for bus ID
            bus_id = index_to_bus_id[iBus]
            if bus_id in slack:
                file.write(f"\t// Dynamic variables for slack bus {bus_id}\n")
                if includeFrequencyRegulation:
                    if limitGenPowers and not usePIforFreq:
                        file.write(f"\t{delta}_g{bus_id}; {omega}_g{bus_id} = {omega}_ref; Eq_p_g{bus_id}; Ed_p_g{bus_id}; Efd_g{bus_id}; Rf_g{bus_id}; VR_g{bus_id}; P_gmr_{bus_id}; P_gml_{bus_id}\n")
                    elif limitGenPowers and usePIforFreq:
                        file.write(f"\t{delta}_g{bus_id}; {omega}_g{bus_id} = {omega}_ref; Eq_p_g{bus_id}; Ed_p_g{bus_id}; Efd_g{bus_id}; Rf_g{bus_id}; VR_g{bus_id}; regI_{bus_id}; P_gml_{bus_id}\n")
                    elif usePIforFreq:
                        file.write(f"\t{delta}_g{bus_id}; {omega}_g{bus_id} = {omega}_ref; Eq_p_g{bus_id}; Ed_p_g{bus_id}; Efd_g{bus_id}; Rf_g{bus_id}; VR_g{bus_id}; regI_{bus_id}; P_gmr_{bus_id}\n")
                    else:
                        file.write(f"\t{delta}_g{bus_id}; {omega}_g{bus_id} = {omega}_ref; Eq_p_g{bus_id}; Ed_p_g{bus_id}; Efd_g{bus_id}; Rf_g{bus_id}; VR_g{bus_id}; P_gmr_{bus_id}\n")
                else:
                    file.write(f"\t{delta}_g{bus_id}; {omega}_g{bus_id} = {omega}_ref; Eq_p_g{bus_id}; Ed_p_g{bus_id}; Efd_g{bus_id}; Rf_g{bus_id}; VR_g{bus_id}\n")                
                file.write(f"\tId_g{bus_id}; Iq_g{bus_id}; Pe_g{bus_id}\n")
            elif bus_id in pv_nodes:
                file.write(f"\t// Dynamic variables for PV bus {bus_id}\n")
                if includeFrequencyRegulation:
                    if limitGenPowers and not usePIforFreq:
                        file.write(f"\t{delta}_g{bus_id}; {omega}_g{bus_id} = {omega}_ref; Eq_p_g{bus_id}; Ed_p_g{bus_id}; Efd_g{bus_id}; Rf_g{bus_id}; VR_g{bus_id}; P_gmr_{bus_id}; P_gml_{bus_id}\n")
                    elif limitGenPowers and usePIforFreq:
        ...
        for real_bus_id in slack:
            ...
            v_angle = np.deg2rad(Va_deg)
            if converter_type == "polar":
                file.write(f"\t{V_angle}_{real_bus_id}_init = {v_angle} [out=true]; ")
                file.write(f"\t{V_mag}_{real_bus_id}_init = {v_slack} [out=true]\n")
            elif converter_type == "rectangular":
                file.write(f"\t{e_var}_{real_bus_id}_init = {v_slack*np.cos(v_angle)} [out=true]; ")
                file.write(f"\t{f_var}_{real_bus_id}_init = {v_slack*np.sin(v_angle)} [out=true]\n")
            elif converter_type == "complex":
                file.write(f"\t{v_cplx}_{real_bus_id}_init = {v_slack}*e^(1i*{v_angle}) [out=true]\n")
        ...
        if convertLoadsToImpedance or useCurrentBalance:
            Vm = np.array([row[7] for row in bus])
            # Add the equivalent admittance (P - jQ) to the Ybus diagonal
            for i_idx in range(n):
                 if Vm[i_idx] > 1e-6:
                    shunt_admittance = (Pd[i_idx] - 1j * Qd[i_idx]) / (Vm[i_idx]**2) # Vm is not 1 always, but can be ---> small difference
        ...
        # finding a bus to start the on-off simulation (if enabled)
        selected_bus_id = None
        selected_i_idx = None

        if onOffPQSimulation == "rnd":
            indices = list(range(n))
            random.shuffle(indices)
            for i_idx in indices:
                bus_id = index_to_bus_id[i_idx]
                if P_inj[i_idx] != 0 and bus_id in pq_nodes and not useCurrentBalance:
                    selected_bus_id = bus_id
                    selected_i_idx = i_idx
                    break
                    
                if Pd[i_idx] != 0 and bus_id in pq_nodes and useCurrentBalance:
                    selected_bus_id = bus_id
                    selected_i_idx = i_idx
                    break

        elif onOffPQSimulation == "first":
            for i_idx in range(n):
                bus_id = index_to_bus_id[i_idx]
                if P_inj[i_idx] != 0 and bus_id in pq_nodes and not useCurrentBalance:
                    selected_bus_id = bus_id
        ...
        elif onOffPQSimulation == "max":
            maxS = 0
            maxIndex = -1
            for i_idx in range(n):
                bus_id = index_to_bus_id[i_idx]
                if bus_id in pq_nodes:
                    Sload = P_inj[i_idx]*P_inj[i_idx] + Q_inj[i_idx]*Q_inj[i_idx]
                    if maxS < Sload:
                        maxIndex = i_idx
                        maxS = Sload
                if bus_id in pq_nodes and useCurrentBalance:
                    Sload = Pd[i_idx]*Pd[i_idx] + Qd[i_idx]*Qd[i_idx]
                    if maxS < Sload:
                        maxIndex = i_idx
                        maxS = Sload
            if maxIndex >= 0:
                selected_bus_id = index_to_bus_id[maxIndex]
                selected_i_idx = maxIndex

        elif onOffPQSimulation == "min":
            minS = 9999999999
            minIndex = -1
            for i_idx in range(n):
                bus_id = index_to_bus_id[i_idx]
                ...
                if bus_id in pq_nodes and useCurrentBalance:
                    ...
                    if Sload > 0 and minS > Sload:
                        ...
                        minS = Sload
            if minIndex >= 0:
                selected_bus_id = index_to_bus_id[minIndex]
                selected_i_idx = minIndex
        ...
        if selected_bus_id is not None and useCurrentBalance:
            magnitude = abs(Y_ii[selected_i_idx])
            angleRad = np.angle(Y_ii[selected_i_idx])
            file.write(f"\t{Y_mag}_{selected_bus_id}_{selected_bus_id}_reg = {magnitude}; ")
            if angleRad != 0:
                file.write(f"{Y_angle}_{selected_bus_id}_{selected_bus_id}_reg = {angleRad} ")
            file.write("\n")

            shunt_admittance = (Pd[selected_i_idx] - 1j * Qd[selected_i_idx]) / (Vm[selected_i_idx]**2) # Vm is not 1 always, but can be ---> small difference
            faultAdm = Y_ii[selected_i_idx] - shunt_admittance
            magnitude = abs(faultAdm)
            angleRad = np.angle(faultAdm)
            file.write(f"\t{Y_mag}_{selected_bus_id}_{selected_bus_id}_fault = {magnitude}; ")
            if angleRad != 0:
                file.write(f"{Y_angle}_{selected_bus_id}_{selected_bus_id}_fault = {angleRad} ")
            file.write("\n")
        ...
        for i_idx in range(n):
            ...
            if iConverterType == 2:
                ...
                if bus_id in pv_nodes:
                    file.write(f"\tP_{bus_id}_g = {P_gen[i_idx]}; ")
                    file.write(f"Q_{bus_id}_g = {Q_gen[i_idx]}; ")
                    file.write(f"P_{bus_id} = {P_inj[i_idx]}; ")
                    file.write(f"Q_{bus_id} = {Q_inj[i_idx]}\n")
                    if not convertLoadsToImpedance and not useCurrentBalance:
                        if (Pd[i_idx] != 0 or Qd[i_idx] != 0):
                            file.write(f"\tP_{bus_id}_l = {Pd[i_idx]}; ")
                            file.write(f"Q_{bus_id}_l = {Qd[i_idx]}\n")
                elif bus_id in pq_nodes:
                    if P_inj[i_idx] != 0 or Q_inj[i_idx] != 0:     #Omit only if both zero
                        file.write(f"\tP_{bus_id} = {P_inj[i_idx]}\n")
                        file.write(f"\tQ_{bus_id} = {Q_inj[i_idx]}\n")
                elif bus_id in slack:
                    file.write(f"\tP_{bus_id} = {P_inj[i_idx]}; ")
                    file.write(f"Q_{bus_id} = {Q_inj[i_idx]}\n")
                    if not convertLoadsToImpedance and not useCurrentBalance:
                        if Pd[i_idx] != 0 or Qd[i_idx] != 0:
        ...
        for iBus in range(n):
            bus_id = index_to_bus_id[iBus]
            if bus_id in pv_nodes or bus_id in slack:
                file.write(f"\tTm_g{bus_id}; Vref_g{bus_id}\n")

        file.write(f"\tf_s = 50; {omega}_ref = 1; {omega}_s = 2*pi*f_s;\n")
        for iBus in range(n):
            bus_id = index_to_bus_id[iBus]
            if bus_id in pv_nodes or bus_id in slack:
                file.write(f"\t// Dynamic parameters for gen {bus_id}\n")
                file.write(f"\tH_g{bus_id} = 6.4; D_g{bus_id} = 28\n")
                file.write(f"\tXd_g{bus_id} = 0.146; Xd_p_g{bus_id} = 0.0608; Xq_g{bus_id} = 0.0969; Xq_p_g{bus_id} = 0.0969;\n")
                file.write(f"\tTdo_p_g{bus_id} = 8.96; Tqo_p_g{bus_id} = 0.31; Tr_g{bus_id} = 0.02; T_gov_g{bus_id} = 0.3\n")
                file.write(f"\tKa_g{bus_id} = 250.0; Ta_g{bus_id} = 0.02; Ke_g{bus_id} = 1.0; Te_g{bus_id} = 0.8\n")
                file.write(f"\tR_g{bus_id} = 0.05; Rs_g{bus_id} = 0.07; Se_g{bus_id} = 0\n")
                file.write(f"\tKf_g{bus_id} = 0.063; Tf_g{bus_id} = 1 \n\n")

        # Writing Submodel
        file.write(f"SubModel [type=NL name=\"Initialization\" copyPars=-1 eps={pfprecision} pivot=\"Diagonal\"]:\n")
        file.write("Vars [out=true]:\n")
        print("Writing Submodel Vars...")
        for iBus in range(n):
            real_bus_id = index_to_bus_id[iBus]
            if real_bus_id not in slack:
                row = bus[iBus]
                ...
                for real_bus_id1 in slack:
                    if converter_type == "polar":
                        if flatStart:
                            file.write(f"\t{V_angle}_{real_bus_id} = {V_angle}_{real_bus_id1}_init; ")
                            file.write(f"{V_mag}_{real_bus_id} = {V_mag}_{real_bus_id1}_init\n")
                        else:
                            Va_rad = np.deg2rad(Va_deg)
                            file.write(f"\t{V_angle}_{real_bus_id} = {Va_rad}; ")
                            file.write(f"{V_mag}_{real_bus_id} = {Vm}\n")
        file.write("Params:\n")
        print("Writing Submodel Params...")
        for iBus in range(n):
            bus_id = index_to_bus_id[iBus]
            if bus_id in pv_nodes or bus_id in slack:
                file.write(f"\t// Gen {bus_id} initials\n")
                file.write(f"\tP_{bus_id}_init; Q_{bus_id}_init\n")
                file.write(f"\tI_r{bus_id}_init; I_i{bus_id}_init; Eq_r{bus_id}_init; Eq_i{bus_id}_init\n")
                file.write(f"\tI_d{bus_id}_init; I_q{bus_id}_init; V_d{bus_id}_init; V_q{bus_id}_init \n")
                file.write(f"\tE_d{bus_id}_p_init; E_q{bus_id}_p_init; Tm{bus_id}_init; Vt_g{bus_id}_init\n")
                file.write(f"\tEf_d{bus_id}_init; VR{bus_id}_init; Rf{bus_id}_init; Vref{bus_id}_init\n")
            ...
            if bus_id in pq_nodes:
                ...
                if iConverterType == 0: # converter_type == "polar":
                    ...
                    if useSumOfCurrentsForZI and P_inj[i_idx]==0 and Q_inj[i_idx]==0:
                        ...
                        writeDiagPowerTermInPolar(realTerms, bus_id, V_mag, Y_mag, Y_angle, False, True)
                        writeDiagReactivePowerTermInPolar(imagTerms, bus_id, V_mag, Y_mag, Y_angle, False, True)
                        ...
                        for j_idx in adjacentNodes:
                            ...
                            writeOffDiagPowerTermInPolar(realTerms, bus_id, j_bus_id, V_mag, V_angle, Y_mag, Y_angle, isSlack, False, bFirst)
                            writeOffDiagReactivePowerTermInPolar(imagTerms, bus_id, j_bus_id, V_mag, V_angle, Y_mag, Y_angle, isSlack, False, bFirst)
                            bFirst = False
                        assert(not bFirst)
                        ...
                        if useCurrentOnRHS:
                            ...
                            writeDiagPowerTermInPolar(realTerms, bus_id, V_mag, Y_mag, Y_angle, False, False)
                            writeDiagReactivePowerTermInPolar(imagTerms, bus_id, V_mag, Y_mag, Y_angle, False, False)
        ...
        if include_limits or genLimitsSelect: 
                    ...
                    for bus_id in bus_list:
                        ...
                        if iConverterType == 0:
                            ...
                            for j_idx in adjacentNodes:
                                j_bus_id = index_to_bus_id[j_idx]
                                isSlack = (j_bus_id in slack)
                                writeOffDiagReactivePowerTermInPolar(imagTerms, bus_id, j_bus_id, V_mag, V_angle, Y_mag, Y_angle, isSlack, bFirst)
                                bFirst = False
                            assert(not bFirst)
                            file.write("".join(imagTerms))
                            file.write(")\n")
            ...
            for group_name, bus_list in group_to_buses.items():
                ...
                for bus_id in bus_list:
                    ...
                    file.write(f"\t\t\tif Q_{bus_id}<=Q_{bus_id}_g_min [signal=TooLow]:\n")
                    file.write(f"\t\t\t\t cGen{bus_id}Reg=false\n")
                    file.write(f"\t\t\t\t Q_{bus_id}=Q_{bus_id}_g_min\n")

                    # Check and enforce upper limit
                    file.write("\t\t\telse:\n")
                    file.write(f"\t\t\t\tif Q_{bus_id}>=Q_{bus_id}_g_max [signal=TooHigh]:\n")
                    file.write(f"\t\t\t\t\t cGen{bus_id}Reg=false\n")
                    file.write(f"\t\t\t\t\t Q_{bus_id}=Q_{bus_id}_g_max\n")
            ...
            for group_name, bus_list in group_to_buses.items():
                ...
                for bus_id in bus_list:
                    ...
                    for j_idx in adjacentNodes:
                        ...
                        writeOffDiagComplexSumTerm(terms, bus_id, j_bus_id, v_cplx, Y_cplx, bFirst)
                    inner_sum = "".join(terms)
                    file.write(f"\tQ_{bus_id} = imag({v_cplx}_{bus_id} * conj({inner_sum}))\n")

        for i_idx in range(n):
            bus_id = index_to_bus_id[i_idx]

            if bus_id in slack or bus_id in pv_nodes:
                adjacentNodes = nodeBranches[i_idx]
                nAdjucent = len(adjacentNodes)
                if nAdjucent == 0:
                    print(f"ERROR! Bus {bus_id} is not connected to any other node")
                    exit(-1)
                isSlack=False
                k_idx = bus_id_map[bus_id]
                if iConverterType == 0: #converter_type == "polar":
                    # Sum of Powers formulation
                    realTerms = []
                    suffix = "_init" if bus_id in slack else ""
                    suffixGen = "_g" if bus_id in pv_nodes else ""
                    if bus_id in slack:
                        isSlack=True
                    writeDiagPowerTermInPolar(realTerms, bus_id, V_mag, Y_mag, Y_angle, isSlack, False)
                    if bus_id in slack:
                        realTerms.append(f" + {V_mag}_{bus_id}_init * (")
                    else:
                        realTerms.append(f" + {V_mag}_{bus_id} * (")
                    bFirst = True
                    for j_idx in adjacentNodes:
                        j_bus_id = index_to_bus_id[j_idx]
                        isSlack1 = False
                        if j_bus_id in slack:
                            isSlack1=True
                        writeOffDiagPowerTermInPolar(realTerms, bus_id, j_bus_id, V_mag, V_angle, Y_mag, Y_angle, isSlack1, isSlack, bFirst)
                        bFirst = False
                    assert(not bFirst)
                    file.write(f"\tP_{bus_id}_init =")
                    if bus_id in slack:
                        file.write("".join(realTerms))
                        file.write(f")\n")
                    else:
                    ...
                    imagTerms = []
                    writeDiagReactivePowerTermInPolar(imagTerms, bus_id, V_mag, Y_mag, Y_angle, isSlack, False)
                    if bus_id in slack:
                        imagTerms.append(f" + {V_mag}_{bus_id}_init * (")
                    else:
                        imagTerms.append(f" + {V_mag}_{bus_id} * (")
                    bFirst = True
                    for j_idx in adjacentNodes:
                        ...
                        if j_bus_id in slack:
                            isSlack1=True
                        writeOffDiagReactivePowerTermInPolar(imagTerms, bus_id, j_bus_id, V_mag, V_angle, Y_mag, Y_angle, isSlack1, isSlack, bFirst)
                        bFirst = False
                    assert(not bFirst)
                    file.write(f"\tQ_{bus_id}_init =")
                    file.write("".join(imagTerms))
                    file.write(")\n")
                    if bus_id in slack and (Pd[i_idx] != 0 or Qd[i_idx] != 0) and not convertLoadsToImpedance and not useCurrentBalance:
                        file.write(f"\tQ_{bus_id}_init = Q_{bus_id}_init + Q_{bus_id}_l\n")
                        file.write(f"\tP_{bus_id}_init = P_{bus_id}_init + P_{bus_id}_l\n")
                    
                    if bus_id in pv_nodes and (Pd[i_idx] != 0 or Qd[i_idx] != 0) and not convertLoadsToImpedance and not useCurrentBalance:
                        file.write(f"\tQ_{bus_id}_init = Q_{bus_id}_init + Q_{bus_id}_l\n")


                    # Step 1: Compute real and imaginary current components
                    file.write(f"\n\t// Step 1:\n")
                    file.write(f"\tI_r{bus_id}_init = (P_{bus_id}_init*cos({V_angle}_{bus_id}{suffix}) + Q_{bus_id}_init*sin({V_angle}_{bus_id}{suffix}))/V_{bus_id}{suffix};\n")
                    file.write(f"\tI_i{bus_id}_init = (P_{bus_id}_init*sin({V_angle}_{bus_id}{suffix}) - Q_{bus_id}_init*cos({V_angle}_{bus_id}{suffix}))/V_{bus_id}{suffix};\n")

                    # Step 2: Compute q-axis EMF and rotor angle
                    file.write(f"\t// Step 2:\n")
                    file.write(f"\tEq_r{bus_id}_init = V_{bus_id}{suffix}*cos({V_angle}_{bus_id}{suffix}) + Rs_g{bus_id}*I_r{bus_id}_init - Xq_g{bus_id}*I_i{bus_id}_init\n")
                    file.write(f"\tEq_i{bus_id}_init = V_{bus_id}{suffix}*sin({V_angle}_{bus_id}{suffix}) + Rs_g{bus_id}*I_i{bus_id}_init + Xq_g{bus_id}*I_r{bus_id}_init\n")
                    file.write(f"\t@main.δ_g{bus_id} = atg2(Eq_r{bus_id}_init, Eq_i{bus_id}_init)\n")

                    # Step 3: Transform currents and voltages to d-q frame
                    file.write(f"\t// Step 3:\n")
                    file.write(f"\tI_d{bus_id}_init = I_r{bus_id}_init*sin(@main.δ_g{bus_id}) - I_i{bus_id}_init*cos(@main.δ_g{bus_id})\n")
                    file.write(f"\tI_q{bus_id}_init = I_r{bus_id}_init*cos(@main.δ_g{bus_id}) + I_i{bus_id}_init*sin(@main.δ_g{bus_id})\n")
                    file.write(f"\tV_d{bus_id}_init = V_{bus_id}{suffix}*sin(@main.δ_g{bus_id} - {V_angle}_{bus_id}{suffix})\n")
                    file.write(f"\tV_q{bus_id}_init = V_{bus_id}{suffix}*cos(@main.δ_g{bus_id} - {V_angle}_{bus_id}{suffix})\n")

                    # Step 4: d-axis transient EMF
                    file.write(f"\t// Step 4:\n")
                    file.write(f"\tE_d{bus_id}_p_init = (Xq_g{bus_id} - Xq_p_g{bus_id}) * I_q{bus_id}_init\n")

                    # Step 5: q-axis transient EMF
                    file.write(f"\t// Step 5:\n")
                    file.write(f"\tE_q{bus_id}_p_init = V_q{bus_id}_init + Rs_g{bus_id} * I_q{bus_id}_init + Xd_p_g{bus_id} * I_d{bus_id}_init\n")

                    # Step 6: Field voltage
                    file.write(f"\t// Step 6:\n")
                    file.write(f"\tEf_d{bus_id}_init = E_q{bus_id}_p_init + (Xd_g{bus_id} - Xd_p_g{bus_id})*I_d{bus_id}_init\n")

                    # Step 7: Exciter states
                    file.write(f"\t// Step 7:\n")
                    file.write(f"\tVR{bus_id}_init = (Ke_g{bus_id} + Se_g{bus_id}) * Ef_d{bus_id}_init\n")
                    file.write(f"\tRf{bus_id}_init = (Kf_g{bus_id} / Tf_g{bus_id}) * Ef_d{bus_id}_init\n")
                    file.write(f"\tVt_g{bus_id}_init = V_{bus_id}{suffix}\n")
                    file.write(f"\tVref{bus_id}_init = V_{bus_id}{suffix} + (VR{bus_id}_init / Ka_g{bus_id})\n")

                    # Assign state variables
                    file.write(f"\t@main.ω_g{bus_id} = ω_ref\n")
                    file.write(f"\t@main.Ed_p_g{bus_id} = E_d{bus_id}_p_init\n")
                    file.write(f"\t@main.Eq_p_g{bus_id} = E_q{bus_id}_p_init\n")
                    file.write(f"\t@main.VR_g{bus_id} = VR{bus_id}_init\n")
                    file.write(f"\t@main.Rf_g{bus_id} = Rf{bus_id}_init\n")
                    file.write(f"\t@main.Id_g{bus_id} = I_d{bus_id}_init\n")
                    file.write(f"\t@main.Iq_g{bus_id} = I_q{bus_id}_init\n")
                    file.write(f"\t@main.Efd_g{bus_id} = Ef_d{bus_id}_init\n")
                    file.write(f"\t@main.Vref_g{bus_id} = Vref{bus_id}_init\n")

                    # Mechanical power (torque balance)
                    file.write(f"\tTm{bus_id}_init = @main.Ed_p_g{bus_id}*I_d{bus_id}_init + @main.Eq_p_g{bus_id}*I_q{bus_id}_init"
                            f" + (Xq_p_g{bus_id} - Xd_p_g{bus_id})*I_d{bus_id}_init*I_q{bus_id}_init;\n")
                    file.write(f"\t@main.Pe_g{bus_id} = Tm{bus_id}_init;\n")
                    if includeFrequencyRegulation:
                        if not usePIforFreq:
                            file.write(f"\t@main.P_gmr_{bus_id} = Tm{bus_id}_init;\n")
                            if limitGenPowers:
                                file.write(f"\t@main.P_gml_{bus_id} = Tm{bus_id}_init;\n")
                        else:
                            if limitGenPowers:
                                file.write(f"\t@main.P_gml_{bus_id} = Tm{bus_id}_init;\n")
                            else:
                                file.write(f"\t@main.P_gmr_{bus_id} = Tm{bus_id}_init;\n")
                            file.write(f"\t@main.regI_{bus_id} = Tm{bus_id}_init;\n") # sta staviti kao poc vrijednost? 0?
                    file.write(f"\t@main.Tm_g{bus_id} = Tm{bus_id}_init;\n")
                    file.write(f"\t@main.Q_{bus_id}{suffixGen} = Q_{bus_id}_init\n")
                    file.write(f"\t@main.P_{bus_id}{suffixGen} = P_{bus_id}_init\n")
                    if bus_id in pv_nodes and (Pd[i_idx] != 0 or Qd[i_idx] != 0) and not convertLoadsToImpedance and not useCurrentBalance:
                        file.write(f"\tQ_{bus_id} = Q_{bus_id}_init - Q_{bus_id}_l\n")
                    file.write(f"\n")
        ...
        for iBus in range(n):
            ...
            if real_bus_id not in slack:
                if iConverterType == 0:
                    file.write(f"\t@main.{V_angle}_{real_bus_id} = {V_angle}_{real_bus_id}; @main.{V_mag}_{real_bus_id} = {V_mag}_{real_bus_id}\n")
            elif real_bus_id in slack:
                if iConverterType == 0:
                    file.write(f"\t@main.{V_angle}_{real_bus_id} = {V_angle}_{real_bus_id}_init; @main.{V_mag}_{real_bus_id} = {V_mag}_{real_bus_id}_init\n")
        ...
        for iBus in range(n):
            ...
            if bus_id in slack or bus_id in pv_nodes:
                ...
                # 7.1 - q-axis transient EMF dynamics
                file.write(f"\tEq_p_g{bus_id}' = (-Eq_p_g{bus_id} - (Xd_g{bus_id} - Xd_p_g{bus_id})*Id_g{bus_id} + Efd_g{bus_id})/Tdo_p_g{bus_id}\n")

                # 7.2 - d-axis transient EMF dynamics
                file.write(f"\tEd_p_g{bus_id}' = (-Ed_p_g{bus_id} + (Xq_g{bus_id} - Xq_p_g{bus_id})*Iq_g{bus_id})/Tqo_p_g{bus_id}\n")

                # 7.3 - rotor angle dynamics
                file.write(f"\tδ_g{bus_id}'    = (ω_g{bus_id} - ω_ref) * ω_s\n")

                # 7.4 - rotor speed dynamics (swing equation)
                if includeFrequencyRegulation:
                    if limitGenPowers:
                        file.write(f"\tω_g{bus_id}'    = (P_gml_{bus_id} - (Ed_p_g{bus_id} * Id_g{bus_id} + Eq_p_g{bus_id} * Iq_g{bus_id} + "
                        f"(Xq_p_g{bus_id} - Xd_p_g{bus_id}) * Id_g{bus_id} * Iq_g{bus_id}) - D_g{bus_id}*(ω_g{bus_id} - ω_ref)) * ω_s /(2*H_g{bus_id})\n")
                    else:
                        file.write(f"\tω_g{bus_id}'    = (P_gmr_{bus_id} - (Ed_p_g{bus_id} * Id_g{bus_id} + Eq_p_g{bus_id} * Iq_g{bus_id} + "
                        f"(Xq_p_g{bus_id} - Xd_p_g{bus_id}) * Id_g{bus_id} * Iq_g{bus_id}) - D_g{bus_id}*(ω_g{bus_id} - ω_ref)) * ω_s /(2*H_g{bus_id})\n")
                else:
                    file.write(f"\tω_g{bus_id}'    = (Tm_g{bus_id} - (Ed_p_g{bus_id} * Id_g{bus_id} + Eq_p_g{bus_id} * Iq_g{bus_id} + "
                        f"(Xq_p_g{bus_id} - Xd_p_g{bus_id}) * Id_g{bus_id} * Iq_g{bus_id}) - D_g{bus_id}*(ω_g{bus_id} - ω_ref)) * ω_s /(2*H_g{bus_id})\n")
                # 7.5 - field voltage (exciter) dynamics
                file.write(f"\tEfd_g{bus_id}'  = (-(Ke_g{bus_id} + Se_g{bus_id})*Efd_g{bus_id} + VR_g{bus_id})/Te_g{bus_id}\n")

                # 7.6 - exciter feedback filter dynamics
                file.write(f"\tRf_g{bus_id}'   = (-Rf_g{bus_id} + (Kf_g{bus_id}/Tf_g{bus_id})*Efd_g{bus_id})/Tf_g{bus_id}\n")

                # 7.7 - voltage regulator dynamics (AVR)
                file.write(f"\tVR_g{bus_id}'   = (-VR_g{bus_id} + Ka_g{bus_id}*Rf_g{bus_id} - "
                        f"(Ka_g{bus_id}*Kf_g{bus_id}/Tf_g{bus_id})*Efd_g{bus_id} + "
                        f"Ka_g{bus_id}*(Vref_g{bus_id} - V_{bus_id}))/Ta_g{bus_id}\n")
                
                if includeFrequencyRegulation:
                    # 7.8 - governor dynamics
                    if usePIforFreq:
                        file.write(f"\tregI_{bus_id} = Ki*int(ω_ref - ω_g{bus_id})\n")
                    else:
                        file.write(f"\tP_gmr_{bus_id}' = (ω_ref - ω_g{bus_id}) / (T_gov_g{bus_id} * R_g{bus_id} * ω_ref)\n")

        file.write("NLEs:\n")
        file.write("\t// Stator algebraic equations\n")
        iGen = 0
        for iBus in range(n):
            bus_id = index_to_bus_id[iBus]
            if bus_id in slack or bus_id in pv_nodes:
                denom = f"(Rs_g{bus_id}^2 + Xd_p_g{bus_id} * Xq_p_g{bus_id})"
                delta_diff = f"(δ_g{bus_id} - {V_angle}_{bus_id})"

                file.write(f"\tId_g{bus_id} = (Rs_g{bus_id} * (Ed_p_g{bus_id} - V_{bus_id} * sin{delta_diff}) + "
                        f"Xq_p_g{bus_id} * (Eq_p_g{bus_id} - V_{bus_id} * cos{delta_diff})) / {denom}\n")

                file.write(f"\tIq_g{bus_id} = (-Xd_p_g{bus_id} * (Ed_p_g{bus_id} - V_{bus_id} * sin{delta_diff}) + "
                        f"Rs_g{bus_id} * (Eq_p_g{bus_id} - V_{bus_id} * cos{delta_diff})) / {denom}\n")
                
                #IDz: This code works only in case when there is one generator connected on a bus (1:1 relation)
                if limitGenPowers and includeFrequencyRegulation and not usePIforFreq:
                    if (bus_id in pv_nodes) or bus_id in slack:
                        if (gen[iGen][8] != 0) or (gen[iGen][9] != 0):
                            file.write(f"\tP_gml_{bus_id}=lim(P_gmr_{bus_id}, {gen[iGen][9]}, {gen[iGen][8]})\n")
                        else:
                            file.write(f"\tP_gml_{bus_id}=lim(P_gmr_{bus_id}, -10, 10)\n")
                        iGen += 1

                if includeFrequencyRegulation and usePIforFreq:
                    if (bus_id in pv_nodes) or bus_id in slack:
                        if limitGenPowers:
                            if (gen[iGen][8] != 0) or (gen[iGen][9] != 0):
                                file.write(f"\tP_gml_{bus_id} = lim(Kp*(ω_ref - ω_g{bus_id}) + regI_{bus_id}, {gen[iGen][9]}, {gen[iGen][8]})\n")
                            else:
                                file.write(f"\tP_gml_{bus_id} = lim(Kp*(ω_ref - ω_g{bus_id}) + regI_{bus_id}, -10, 10)\n")
                        else:
                            file.write(f"\tP_gmr_{bus_id} = Kp*(ω_ref - ω_g{bus_id}) + regI_{bus_id}\n")
                        iGen += 1
        ...
        for iBus in range(n):
            bus_id = index_to_bus_id[iBus]
            if bus_id in slack or bus_id in pv_nodes:
                file.write(f"\tPe_g{bus_id} = Ed_p_g{bus_id} * Id_g{bus_id} + Eq_p_g{bus_id} * Iq_g{bus_id} + "
                   f"(Xq_p_g{bus_id} - Xd_p_g{bus_id}) * Id_g{bus_id} * Iq_g{bus_id}\n")
        ...
        if useCurrentBalance:
            for i_idx in range(n):
                ...
                if bus_id in pv_nodes or bus_id in slack:
                    j_idx = bus_id_map[bus_id]
                    if comment_equations:
                        file.write(f"\t// node {bus_id} - PV\n")
                                
                    realTerms = []
                    imagTerms = []          

                    writeDiagPowerTermInCurrentPolar(realTerms, bus_id, V_mag, V_angle, Y_mag, Y_angle, False, True)
                    writeDiagReactivePowerTermInCurrentPolar(imagTerms, bus_id, V_mag, V_angle, Y_mag, Y_angle, False, True)
                    realTerms.append(" + ")
                    imagTerms.append(" + ")
                    bFirst = True
                    for j_idx in adjacentNodes:
                        j_bus_id = index_to_bus_id[j_idx]
                        isSlack = (j_bus_id in slack)
                        writeOffDiagPowerTermInCurrentPolar(realTerms, bus_id, j_bus_id, V_mag, V_angle, Y_mag, Y_angle, isSlack, False, bFirst)
                        writeOffDiagReactivePowerTermInCurrentPolar(imagTerms, bus_id, j_bus_id, V_mag, V_angle, Y_mag, Y_angle, isSlack, False, bFirst)
                        bFirst = False
                    assert(not bFirst)

                    file.write(f"\tId_g{bus_id}*sin(δ_g{bus_id}) + Iq_g{bus_id}*cos(δ_g{bus_id}) =")
                    file.write("".join(realTerms))
                    file.write("\n")
                    file.write(f"\tIq_g{bus_id}*sin(δ_g{bus_id}) - Id_g{bus_id}*cos(δ_g{bus_id}) =")
                    file.write("".join(imagTerms))
                    file.write("\n")
        else:
            for i_idx in range(n):
                ...
                if bus_id in pv_nodes or bus_id in slack:
                    ...
                    if iConverterType == 0: 
                        realTerms = []
                        writeDiagPowerTermInPolar(realTerms, bus_id, V_mag, Y_mag, Y_angle, False, False)
                        realTerms.append(f" + {V_mag}_{bus_id} * (".replace('\t',''))

                        imagTerms = []
                        writeDiagReactivePowerTermInPolar(imagTerms, bus_id, V_mag, Y_mag, Y_angle, False, False)
                        imagTerms.append(f" + {V_mag}_{bus_id} * (".replace('\t',''))
                        bFirst = True
                        for j_idx in adjacentNodes:
                            j_bus_id = index_to_bus_id[j_idx]
                            writeOffDiagPowerTermInPolar(realTerms, bus_id, j_bus_id, V_mag, V_angle, Y_mag, Y_angle, False, False, bFirst)
                            writeOffDiagReactivePowerTermInPolar(imagTerms, bus_id, j_bus_id, V_mag, V_angle, Y_mag, Y_angle, False, False, bFirst)
                            bFirst = False
                        assert(not bFirst)
                        delta_diff = f"(δ_g{bus_id} - {V_angle}_{bus_id})"

                        if (Pd[i_idx] != 0 or Qd[i_idx] != 0) and not convertLoadsToImpedance:
                            file.write(f"\tV_{bus_id} * (Id_g{bus_id}*sin{delta_diff} + Iq_g{bus_id}*cos{delta_diff}) - P_{bus_id}_l=")
                        else:
                            file.write(f"\tV_{bus_id} * (Id_g{bus_id}*sin{delta_diff} + Iq_g{bus_id}*cos{delta_diff}) =")
                        file.write("".join(realTerms))
                        file.write(")\n")

                        if (Pd[i_idx] != 0 or Qd[i_idx] != 0) and not convertLoadsToImpedance:
                            file.write(f"\tV_{bus_id} * (Id_g{bus_id}*cos{delta_diff} - Iq_g{bus_id}*sin{delta_diff}) - Q_{bus_id}_l=")
                        else:
                            file.write(f"\tV_{bus_id} * (Id_g{bus_id}*cos{delta_diff} - Iq_g{bus_id}*sin{delta_diff}) =")
                        file.write("".join(imagTerms))
                        file.write(")\n")
        ...
        for i_idx in range(n):
            ...
            if bus_id in pq_nodes:
                ...
                if useCurrentBalance:
                    ...
                    for j_idx in adjacentNodes:
                        j_bus_id = index_to_bus_id[j_idx]
                        writeOffDiagPowerTermInCurrentPolar(realTerms, bus_id, j_bus_id, V_mag, V_angle, Y_mag, Y_angle, False, False, bFirst)
                        writeOffDiagReactivePowerTermInCurrentPolar(imagTerms, bus_id, j_bus_id, V_mag, V_angle, Y_mag, Y_angle, False, False, bFirst)
                        bFirst = False
                    assert(not bFirst)
        ...
        if onOffPQSimulation == "rnd":
            ...
            for i_idx in indices:
                ...
                if P_inj[i_idx] != 0 and bus_id in pq_nodes:
                    writePostProcBlock(file, bus_id, P_inj[i_idx], Q_inj[i_idx], onOffPQSimulation)
                    break
        elif onOffPQSimulation == "first":   #find first PQ node with load
            for i_idx in range(n):
                bus_id = index_to_bus_id[i_idx]
                adjacentNodes = nodeBranches[i_idx]
                if P_inj[i_idx] != 0:
                    if bus_id in pq_nodes:
                        writePostProcBlock(file, bus_id, P_inj[i_idx], Q_inj[i_idx], onOffPQSimulation)
                        break
        elif onOffPQSimulation == "max":   #find the largest PQ load
            maxS = 0
            maxIndex = -1
            for i_idx in range(n):
                bus_id = index_to_bus_id[i_idx]
                if bus_id in pq_nodes:
                    ...
                    if maxS < Sload:
                        ...
                        maxS = Sload
            if maxIndex >= 0:
                bus_id = index_to_bus_id[maxIndex]
                adjacentNodes = nodeBranches[maxIndex]
                writePostProcBlock(file, bus_id, P_inj[maxIndex], Q_inj[maxIndex], onOffPQSimulation)

        elif onOffPQSimulation == "min":   #find the largest PQ load
            minS = 9999999999
            minIndex = -1
            for i_idx in range(n):
                bus_id = index_to_bus_id[i_idx]
                if bus_id in pq_nodes:
                    ...
                    if Sload > 0:
                        if minS > Sload:
                            ...
                            minS = Sload
            if minIndex >= 0:
                bus_id = index_to_bus_id[minIndex]
                adjacentNodes = nodeBranches[minIndex]
                writePostProcBlock(file, bus_id, P_inj[minIndex], Q_inj[minIndex])
        '''
        # writing post-processing block for on-off PQ simulation (if enabled)
        if useCurrentBalance:
            if selected_bus_id is not None:
                adjacentNodes = nodeBranches[selected_i_idx]
                writePostProcCurrentBlock(file, selected_bus_id, P_inj[selected_i_idx], Q_inj[selected_i_idx], onOffPQSimulation, Y_angle, Y_mag)
            file.write("end\n")                    
        else:
            if selected_bus_id is not None:
                adjacentNodes = nodeBranches[selected_i_idx]
                writePostProcBlock(file, selected_bus_id, P_inj[selected_i_idx], Q_inj[selected_i_idx], onOffPQSimulation)
            file.write("end\n")                    
    ...
    print("... Writing visual model ...")
    colorsDark = [
        # Maximum contrast starters (very distinct hues)
        "white", "magenta", "cyan", "yellow", "orange", "lime", "lightBlue", "hotPink",

        # Strong secondary contrasts
        "gold", "violet", "aqua", "coral", "springGreen", "dodgerBlue", "pink", "turquoise",

        # Medium contrast group
        "lightGreen", "orchid", "deepSkyBlue", "salmon", "mediumSpringGreen", "plum", "skyBlue", "tangerine",

        # Softer but still visible
        "paleGreen", "mediumTurquoise", "lightSalmon", "lightSkyBlue", "mint", "paleTurquoise",

        # Lightest / lowest contrast (use for many lines)
        "lightCyan", "lemonChiffon", "lightYellow"]

    colorsLight = [
        # Maximum contrast starters
        "black", "darkRed", "darkBlue", "darkGreen", "purple", "saddleBrown", "indigo", "darkMagenta",

        # Strong secondary contrasts
        "maroon", "navy", "forestGreen", "royalBlue", "eggplant", "olive", "crimson", "midnightBlue",

        # Medium contrast
        "seaGreen", "firebrick", "pine", "mulberry", "oliveDrab", "sienna", "chocolate",

        # Lowest contrast (use for many lines)
        "sepia", "bronze", "dimGray", "darkSlateGray"]
    
    linePatterns = ["solid", "dash", "dot", "dashDot", "dashDotDot"]
                    
    vmodl_path = dmodl_output_path + "_dyn.vmodl"
    with open(vmodl_path, 'w', encoding='utf-8') as vfile:

        vfile.write("Header:\n")
        vfile.write("\tnewTab = false\n")
        vfile.write("end\n")
        vfile.write(f"Model [name=\"Visual of Dynamics\"]:\n")
        vfile.write("Plots:\n")

        # --- Frequency plot ---
        vfile.write("\tlinePlot [xLabel=\"Time [s]\" yLabel=\"Speed [p.u.]\" name=\"Gen. speeds\"]:\n")
        vfile.write("\t\t@x << t\n")
        iPlot = 0
        lineWidth = 2
        for iBus in range(n):
            bus_id = index_to_bus_id[iBus]
            if bus_id in slack or bus_id in pv_nodes:
                colorD = colorsDark[iPlot % len(colorsDark)]
                colorL = colorsLight[iPlot % len(colorsLight)]
                iLp = iPlot // len(colorsDark)
                if iLp >= len(linePatterns):
                    iLp = 4
                    lineWidth = 2
                linePattern = linePatterns[iLp]
                vfile.write(f"\t\t@y << ω_g{bus_id} [colorL={colorL} colorD={colorD} width={lineWidth} pattern=\"{linePattern}\" name=\"ω_g{bus_id}\"]\n")
                iPlot += 1
        vfile.write("\t\t@y << ω_ref [colorL=black colorD=white width=1 pattern=\"dot\" name=\"Ref. ω\"]\n")
        vfile.write("\tend\n\n")

        # --- Governor power plot ---
        vfile.write("\tlinePlot [xLabel=\"Time [s]\" yLabel=\"Power [p.u.]\" name=\"Gen. Powers\"]:\n")
        vfile.write("\t\t@x << t\n")
        iPlot = 0
        for iBus in range(n):
            bus_id = index_to_bus_id[iBus]
            if bus_id in slack or bus_id in pv_nodes:
                ...
                if includeFrequencyRegulation:
                    if limitGenPowers:
                        vfile.write(f"\t\t@y << P_gml_{bus_id} [colorL={colorL} colorD={colorD} width=2 name=\"Pgm_{bus_id}\"]\n")
                    else:
                        vfile.write(f"\t\t@y << P_gmr_{bus_id} [colorL={colorL} colorD={colorD} width=2 name=\"Pgm_{bus_id}\"]\n")
                else:
                    vfile.write(f"\t\t@y << Tm_g{bus_id} [colorL={colorL} colorD={colorD} width=2 name=\"Tm_g{bus_id}\"]\n")
                iPlot += 1
        if not plotMechPowerOnly:
            for iBus in range(n):
                bus_id = index_to_bus_id[iBus]
                if bus_id in slack or bus_id in pv_nodes:
                    colorD = colorsDark[iBus % len(colorsDark)]
                    colorL = colorsLight[iBus % len(colorsLight)]
                    vfile.write(f"\t\t@y << Pe_g{bus_id} [colorL={colorL} colorD={colorD} width=2 pattern=\"dot\" name=\"Pe_g{bus_id}\"]\n")
        vfile.write("\tend\n\n")

        # --- Rotor angle plot ---
        vfile.write("\tlinePlot [xLabel=\"Time [s]\" yLabel=\"Angle [rad]\" name=\"Rotor Angles\"]:\n")
        vfile.write("\t\t@x << t\n")
        iPlot = 0
        for iBus in range(n):
            bus_id = index_to_bus_id[iBus]
            if bus_id in slack or bus_id in pv_nodes:
                ...
                if iLp >= len(linePatterns):
                    ...
                    lineWidth = 2
                linePattern = linePatterns[iLp]
                vfile.write(f"\t\t@y << δ_g{bus_id} [colorL={colorL} colorD={colorD} width={lineWidth} pattern=\"{linePattern}\" name=\"δ_g{bus_id}\"]\n")
                iPlot += 1
        vfile.write("\tend\n\n")

        # --- Terminal voltage plot ---
        vfile.write("\tlinePlot [xLabel=\"Time [s]\" yLabel=\"Voltage [p.u.]\" name=\"Gen. Bus Voltages\"]:\n")
        vfile.write("\t\t@x << t\n")
        iPlot = 0
        for iBus in range(n):
            bus_id = index_to_bus_id[iBus]
            if bus_id in slack or bus_id in pv_nodes:
                colorD = colorsDark[iPlot % len(colorsDark)]
                colorL = colorsLight[iPlot % len(colorsLight)]
                vfile.write(f"\t\t@y << V_{bus_id} [colorL={colorL}  colorD={colorD} width=2 name=\"Vt_g{bus_id}\"]\n")
                iPlot += 1
        vfile.write("\tend\n\n")
