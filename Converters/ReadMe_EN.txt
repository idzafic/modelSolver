...
Python 3.9 or higher is recommended for full compatibility.
Required Libraries:
...
Usage:
	-Prepare the XML configuration file specifying your MATPOWER .m input file and script options.
		1.) File Section:
			In this section users can specify the full path to original MATPOWER input file (.m file) and the base name
...
To run the script from the terminal, type the following command where "X" is the number of the case file:
    >>> python matp2PFmodl.py caseX.m

You can also specify a custom name for the output file:
    >>> python matp2PFmodl.py caseX.m -o nameOfOutputFile

If you would like just to specify output folder (while reusing input file name with changed extension):
    >>> python matp2PFmodl.py caseX.m -resPath=./res
or
    >>> python matp2PFmodl.py caseX.m --r=./res
...
    >>> python matp2PFmodl.py --help

For state estimation use matp2SEmodl.py.
