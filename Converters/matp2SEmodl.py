...
# Generating string for PF
def generate_inner_sum_string(i_idx, bus_id, nodeBranches, index_to_bus_id, v_cplx, Y_cplx):
...
def build_inner_sum(bus_id, adjacentNodes, index_to_bus_id, v_cplx, Y_cplx):
    terms = []
    writeDiagComplexSumTerm(terms, bus_id, v_cplx, Y_cplx)
    bFirst = False 
    for j_idx in adjacentNodes:
...
def get_branch_suffix(bus, parallel_branch_keys, suffix_tracker):
    from_bus = int(bus[0])
    to_bus = int(bus[1])
    branch_key = tuple(sorted((from_bus, to_bus)))
    suffix = ""
    if branch_key in parallel_branch_keys:
        current_num = suffix_tracker.get(branch_key, 0) + 1
        suffix = f"_{current_num}"
        suffix_tracker[branch_key] = current_num
    return suffix, from_bus, to_bus
...
def main():
    ...
    if debug_mode:
        ...
        matpower_file = "case9241pegase.m" # "case_ACTIVSg25k.m" "case118.m" #"case3375wp.m" #"case_SyntheticUSA.m" #"case6495rte.m" #"case6468rte.m" #"case300.m"
    ...
    if output_path:
        ...
        if res_path:
            ...
            dmodl_output_path = file_name_without_ext + "_SE"
    ...
    includeNoise = optionsES.find('includeNoise').text.strip().lower() == 'true'
    noiseWidthCoef = float(optionsES.find('noiseWidthCoef').text)
    ...
    if not slack:
        raise ValueError("The 'slack' array cannot be empty.")
    slack_bus_id = slack[0] 
    num_to_select = int(n * voltage_meas)
    all_real_bus_ids = list(bus_id_map.keys())

    if num_to_select <= 0:
        random_voltage = []
    elif num_to_select >= n:
        random_voltage = sorted(all_real_bus_ids)
    elif num_to_select == 1:
        random_voltage = [slack_bus_id]
    else:
        candidate_buses = [bus_id for bus_id in all_real_bus_ids if bus_id != slack_bus_id]
        num_to_randomly_select = num_to_select - 1
        if num_to_randomly_select > len(candidate_buses):
            randomly_selected_buses = candidate_buses
        else:
            randomly_selected_buses = random.sample(candidate_buses, num_to_randomly_select)
        final_selected_buses = [slack_bus_id] + randomly_selected_buses
        random_voltage = sorted(final_selected_buses)
    ...
    if loadTransferSelect:
        resFileName = dmodl_output_path + "_SE_lt.dmodl"
        ...
        resFileName = dmodl_output_path + "_SE.dmodl"

    
    if genLimitsSelect:
        resFileName = dmodl_output_path + "_SE_pv.dmodl"
    ...
    with open(resFileName, "w", encoding='utf-8') as file:
        ...
        file.write(f"Model [type=WLS reInit=true eps={pfprecision} domain=cmplx name=\"SE With PF SubModel in {converter_type} coordinates\"]:\n")
            ...
            for real_bus_id1 in slack:
                if converter_type == "polar":
                    ...
                    if flatStart:
                        ...
                        file.write(f"\t{v_cplx}_{real_bus_id} = {Va_real}+{Va_imag}i\n")
        ...
        all_branch_keys = [tuple(sorted((int(b[0]), int(b[1])))) for b in branch]
        parallel_branch_keys = {key for key in all_branch_keys if all_branch_keys.count(key) > 1}
        suffix_tracker = {}

        for bus_pair in random_branches:
            from_bus = int(bus_pair[0])
            to_bus = int(bus_pair[1])
            branch_key = tuple(sorted((from_bus, to_bus)))
            
            suffix = ""
            if branch_key in parallel_branch_keys:
                current_suffix_num = suffix_tracker.get(branch_key, 0) + 1
                suffix = f"_{current_suffix_num}"
                suffix_tracker[branch_key] = current_suffix_num

            if converter_type == "complex":
                file.write(f"\tS_{from_bus}_{to_bus}_meas{suffix} [out = true]\n")
                file.write(f"\tS_{from_bus}_{to_bus}_est{suffix} [out = true]\n")
            else: 
                file.write(f"\tP_{from_bus}_{to_bus}_meas{suffix} [out = true]; ")
                file.write(f"P_{from_bus}_{to_bus}_est{suffix} [out = true]\n")
                file.write(f"\tQ_{from_bus}_{to_bus}_meas{suffix} [out = true]; ")
                file.write(f"Q_{from_bus}_{to_bus}_est{suffix} [out = true]\n")

        
        # Adding measuring params
        #print(slack)
        #print (index_to_bus_id)
        for bus_id in random_voltage:
            #print(bus_id)
            #real_bus_id = index_to_bus_id[bus_id - 1]
            if converter_type == "complex":
                file.write(f"\t{v_cplx}_{bus_id}_meas [type=real out=true]\n")
            else:
                file.write(f"\t{V_mag}_{bus_id}_meas [out=true]\n")
        if converter_type == "complex":
        ...
        for i_idx in range(n):
            ...
            # Write P and Q for PV nodes
            if bus_id in random_pv_nodes:
                if P_inj[i_idx] != 0:
                    file.write(f"\tP_{bus_id}_g_meas [out = true]; P_{bus_id}_g_est [out = true]\n\tQ_{bus_id}_g_est [out = true]\n")

        # SUBMODEL ----------------------------------------------> Writing submodel in model 
        print(".... starting submodel ...")
        if iConverterType == 2:
            file.write(f"SubModel [type=NL alwaysOn=true eps={pfprecision} maxIter=200 domain=cmplx copyPars=-1 alwaysOn=false name=\"Solving power flow\"]:\n")
            file.write("\tVars [conj=true out=true]:\n")
        else:
            file.write(f"SubModel [type=NL alwaysOn=true eps={pfprecision} maxIter=200 domain=real copyPars=-1 alwaysOn=false name=\"Solving power flow\"]:\n")
            file.write("\tVars [out=true]:\n")
        ...
        for real_bus_id in slack:
            ...
            for real_bus_id1 in slack:
                if converter_type == "polar":
                    ...
                    file.write(f"\t\t{v_cplx}_{real_bus_id} = {v_cplx}_{real_bus_id1}_sl\n")
        file.write("\tDistribs:\n")
        file.write(f"\t\tg_inj [type=Gauss mean=0 dev={inj_deviation} width={inj_deviation*noiseWidthCoef}]\n")
        file.write(f"\t\tg_br [type=Gauss mean=0 dev={branch_deviation} width={branch_deviation*noiseWidthCoef}]\n")
        file.write(f"\t\tg_v [type=Gauss mean=0 dev={voltage_deviation} width={voltage_deviation*noiseWidthCoef}]\n")
      ...
      # Begin writing the nonlinear equations (NLEs) section
            ...
            if bus_id in pq_nodes:
                ...
                if iConverterType == 0: # converter_type == "polar":
                    ...
                    if useSumOfCurrentsForZI and P_inj[i_idx]==0 and Q_inj[i_idx]==0:
                        if comment_equations:
                            file.write(f"\t\t// node {bus_id} - PQ (ZI)\n")
                        inner_sum = build_inner_sum(bus_id, adjacentNodes, index_to_bus_id, v_cplx, Y_cplx)
        ...
        if iConverterType == 2:
            ...
            for group_name, bus_list in group_to_buses.items():
                ...
                for bus_id in bus_list:
                    i_idx = bus_id_map[bus_id]
                    inner_sum = generate_inner_sum_string(i_idx, bus_id, nodeBranches, index_to_bus_id, v_cplx, Y_cplx)
                    file.write(f"\t\tQ_{bus_id}_g = imag({v_cplx}_{bus_id} * conj({inner_sum}))\n")

            for bus_id in random_voltage:
                noise_string = " + real(rnd(g_v))" if includeNoise else ""
                line = f"\t\t@main.{v_cplx}_{bus_id}_meas = abs({v_cplx}_{bus_id}){noise_string}\n"
                file.write(line)
            
            for bus_id in random_pq_nodes:
                i_idx = bus_id_map[bus_id]
                if P_inj[i_idx]!=0 or Q_inj[i_idx]!=0:
                    noise_string = " + rnd(g_inj)" if includeNoise else ""
                    i_idx = bus_id_map[bus_id]
                    inner_sum = generate_inner_sum_string(i_idx, bus_id, nodeBranches, index_to_bus_id, v_cplx, Y_cplx)
                    file.write(f"\t\t@main.S{bus_id}_meas = {v_cplx}_{bus_id} * conj({inner_sum}){noise_string}\n")
            for bus_id in random_pv_nodes:
                i_idx = bus_id_map[bus_id]
                if P_inj[i_idx]!=0 or Q_inj[i_idx]!=0:
                    i_idx = bus_id_map[bus_id]
                    inner_sum = generate_inner_sum_string(i_idx, bus_id, nodeBranches, index_to_bus_id, v_cplx, Y_cplx)
                    noise_string = " + real(rnd(g_inj))" if includeNoise else ""
                    file.write(f"\t\t@main.P_{bus_id}_g_meas = real({v_cplx}_{bus_id} * conj({inner_sum})){noise_string}\n")

            # S between two branches
            all_branch_keys = [tuple(sorted((int(b[0]), int(b[1])))) for b in branch]
            parallel_branch_keys = {key for key in all_branch_keys if all_branch_keys.count(key) > 1}
            suffix_tracker = {}
            for bus in random_branches:
                suffix, from_bus, to_bus = get_branch_suffix(bus, parallel_branch_keys, suffix_tracker)
                noise_string = " + rnd(g_br)" if includeNoise else ""
                file.write(f"\t\t@main.S_{from_bus}_{to_bus}_meas{suffix} = {v_cplx}_{from_bus}*conj({v_cplx}_{from_bus} - {v_cplx}_{to_bus})*conj(-{Y_cplx}_{from_bus}_{to_bus}){noise_string}\n")
...
# -----------------------------------------------------------------------------------> END OF SUBMODEL
        ...
        if iConverterType == 2:
            ...
            for bus_id in random_voltage:
                    if bus_id not in slack:
                        file.write(f"\t[w=w_v] {v_cplx}_{bus_id}*conj({v_cplx}_{bus_id}) = {v_cplx}_{bus_id}_meas^2\n")
            for bus_id in random_pv_nodes:
                i_idx = bus_id_map[bus_id]
                if P_inj[i_idx]!=0 or Q_inj[i_idx]!=0:
                    inner_sum = generate_inner_sum_string(i_idx, bus_id, nodeBranches, index_to_bus_id, v_cplx, Y_cplx)
                    file.write(f"\t[w=w_inj] {v_cplx}_{bus_id} * conj({inner_sum}) + conj({v_cplx}_{bus_id}) * ({inner_sum}) = 2*P_{bus_id}_g_meas\n")
            for bus_id in random_pq_nodes:
                i_idx = bus_id_map[bus_id]
                if P_inj[i_idx]!=0 or Q_inj[i_idx]!=0:
                    inner_sum = generate_inner_sum_string(i_idx, bus_id, nodeBranches, index_to_bus_id, v_cplx, Y_cplx)
                    file.write(f"\t[w=w_inj] {v_cplx}_{bus_id} * conj({inner_sum}) = S{bus_id}_meas\n")
                    file.write(f"\t[w=w_inj] conj({v_cplx}_{bus_id}) * ({inner_sum}) = conj(S{bus_id}_meas)\n")
            ...
            for bus in random_branches:
                suffix, from_bus, to_bus = get_branch_suffix(bus, parallel_branch_keys, suffix_tracker)
                file.write(f"\t[w=w_br] {v_cplx}_{from_bus}*conj({v_cplx}_{from_bus} - {v_cplx}_{to_bus})*conj(-{Y_cplx}_{from_bus}_{to_bus}) = S_{from_bus}_{to_bus}_meas{suffix}\n")
                file.write(f"\t[w=w_br] conj({v_cplx}_{from_bus})*({v_cplx}_{from_bus} - {v_cplx}_{to_bus})*(-{Y_cplx}_{from_bus}_{to_bus}) = conj(S_{from_bus}_{to_bus}_meas{suffix})\n")
            ...
            if estimation_method == "NE":
                for bus_id in pq_nodes:
                    ...
                    if P_inj[i_idx]==0 and Q_inj[i_idx]==0: #sum of currents
                        inner_sum = generate_inner_sum_string(i_idx, bus_id, nodeBranches, index_to_bus_id, v_cplx, Y_cplx)
                        file.write(f"\t[w=w_zi] conj({inner_sum}) = 0\n")
                        file.write(f"\t[w=w_zi] {inner_sum} = 0\n")
        if estimation_method == "EC":
            ...
            if iConverterType == 2:
                for bus_id in pq_nodes:
                    ...
                    if P_inj[i_idx]==0 and Q_inj[i_idx]==0:
                        inner_sum = generate_inner_sum_string(i_idx, bus_id, nodeBranches, index_to_bus_id, v_cplx, Y_cplx)
                        file.write(f"\tconj({inner_sum}) = 0\n")
                        file.write(f"\t{inner_sum} = 0\n")    
        file.write("PostProc:\n")
        if iConverterType == 2:
            for bus_id in random_pq_nodes:
                i_idx = bus_id_map[bus_id]
                if P_inj[i_idx]!=0 or Q_inj[i_idx]!=0:
                    inner_sum = generate_inner_sum_string(i_idx, bus_id, nodeBranches, index_to_bus_id, v_cplx, Y_cplx)
                    file.write(f"\tS{bus_id}_est = {v_cplx}_{bus_id} * conj({inner_sum})\n")        
            for bus_id in random_pv_nodes:
                i_idx = bus_id_map[bus_id]
                if P_inj[i_idx]!=0 or Q_inj[i_idx]!=0:
                    inner_sum = generate_inner_sum_string(i_idx, bus_id, nodeBranches, index_to_bus_id, v_cplx, Y_cplx)
                    file.write(f"\tP_{bus_id}_g_est = real({v_cplx}_{bus_id} * conj({inner_sum}))\n")        
                    file.write(f"\tQ_{bus_id}_g_est = imag({v_cplx}_{bus_id} * conj({inner_sum}))\n")   
            ...
            for bus in random_branches:
                suffix, from_bus, to_bus = get_branch_suffix(bus, parallel_branch_keys, suffix_tracker)
                file.write(f"\tS_{from_bus}_{to_bus}_est{suffix} = {v_cplx}_{from_bus}*conj({v_cplx}_{from_bus} - {v_cplx}_{to_bus})*conj(-{Y_cplx}_{from_bus}_{to_bus})\n")
